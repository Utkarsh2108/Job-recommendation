...
resume_data_path = 'data/concatenated_data/knn_cv.csv'
...
def app():
    ...
    if len(jd) >=1:
        ...
        for i in range(len(df.columns)):
            NLP_Processed_cv=text_preprocessing.nlp(df.columns.values[i])
        ...
        # KNN function
        n_neighbors = min(19, len(df))  # Set n_neighbors to the minimum of 19 or the number of available samples
        top, index_score = distance_calculation.KNN(df['clean_all'], jd_df['jd'], number_of_neighbors=n_neighbors)
