...
preprocessed_jobs_path ="data/concatenated_data/preprocessed_jobs.csv"
Resume_of_candidate_path = "data/concatenated_data/Resume_from_CANDIDATE.csv"
all_locations_path = "data/concatenated_data/all_locations.csv"

st.set_page_config(layout="wide", page_title="CANDIDATE")
...
def app():
    ...
    # career level
    job_loc = pd.read_csv(preprocessed_jobs_path)
        ...
        if st.button('Proceed Further !! '):
            with st_lottie_spinner(url, key="download",    reverse=True,speed=1,loop=True,quality='high',):
                ...
                try:
                    ...
                    if count_==0:
                        count_=1
                        MongoDB_function.resume_store(save,Resume_of_candidate_path)
                    ...
                    df= MongoDB_function.get_collection_as_dataframe(preprocessed_jobs_path)
            ...
            #<<<<<<<<<<<<<<<<<<VISUALIZATION>>>>>>>>>>>>>>>>>>>>>>>
                    ...
                    locations_df['count'] = locations_df['count'].astype(str)
                    locations_df['count'] = locations_df['count'].apply(lambda x: x.replace("Area", "") if "Area" in x else x)
                    ...
                    locations_df['loc_geo'] = locations_df['count'].apply(geocode)
                    ...
                    for lat, lon, ind, job_no in zip(locations_df['latitude'], locations_df['longitude'], locations_df['count'], locations_df['location']):
